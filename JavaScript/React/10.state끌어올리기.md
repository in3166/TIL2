# State 끌어올리기
- ***`state 공유`***
- 동일한 데이터 변경사항을 여러 컴포넌트에 반영해야 할 때 하나의 진리의 원천(source of truth)을 둔다.
- 즉, 하나의 state가 다른 컴포넌트에서도 필요할 때
- 가장 가까운 공통 조상으로 state 끌어올린다.
  - 조상 컴포넌트에서 `Event Handler`를 props로 전달하면 하위 컴포넌트에서 상위 컴포넌트의 state를 제어할 수 있다.
<br />

  - *Boiler Plate: 반드시 작성해햐자미나 반복적인 코드*
<br />
...
## State 끌어올리기
- 두 개의 `TemperatureInput` 컴포넌트가 각각의 입력값을 각자의 state에 독립적으로 저장하고 있다.
