...
- Request와 Response (HTTP의 파이프라인을 구성하는 요소 조작)를 포함한 객체
...
## `jQuery.ajax()`와 다른점
- `fetch()`로 반환되는 Promise 객체는 HTTP error 상태를 reject하지 않음. ( 404, 500 )
- 대신, OK 상태가 false인 resolve 반환, 네트워크 장애나 요청 완료되지 못한 상태는 reject 반환
- 보통 `fetch`는 쿠키를 보내거나 받지 않음 -> 쿠키 전송을 위해 자격증명(credentials) 옵션 설정
